...
kubeadm_init(){
    kubeadm init --pod-network-cidr=192.168.0.0/16  --service-cidr=10.96.0.0/12 --apiserver-advertise-address=${1} --apiserver-cert-extra-sans=kubeapi.pt.ysicing.me --ignore-preflight-errors=Swap | grep "discovery-token" | grep "join" > /tmp/join
    mkdir -p $HOME/.kube
